...
CREATE TABLE IF NOT EXISTS fri_lessors (
  ...
  subscription_plan NVARCHAR(50) DEFAULT 'professional', -- professional, business, enterprise
  subscription_status NVARCHAR(50) DEFAULT 'active', -- active, paused, cancelled
...
CREATE TABLE IF NOT EXISTS fri_lessor_team_members (
  ...
  phone NVARCHAR(20),
  role NVARCHAR(50) NOT NULL, -- owner, admin, manager, driver, mechanic, accountant
  is_active BIT DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS fri_vehicles (
  ...
  status NVARCHAR(50) DEFAULT 'available', -- available, unavailable, maintenance, sold
  odometer INT DEFAULT 0,
  fuel_type NVARCHAR(50), -- gasoline, diesel, electric, hybrid
...
CREATE TABLE IF NOT EXISTS fri_vehicle_maintenance (
  id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
  vehicle_id UNIQUEIDENTIFIER NOT NULL,
  maintenance_type NVARCHAR(100) NOT NULL, -- oil change, inspection, repair, tire
...
-- Customers (renters)
CREATE TABLE IF NOT EXISTS fri_customers (
  id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
  lessor_id UNIQUEIDENTIFIER NOT NULL,
  full_name NVARCHAR(255) NOT NULL,
  email NVARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS fri_bookings (
  ...
  additional_fees DECIMAL(10, 2) DEFAULT 0, -- GPS, insurance addon, cleaning, etc
  total_price DECIMAL(10, 2) NOT NULL,
  status NVARCHAR(50) DEFAULT 'pending', -- pending, confirmed, active, completed, cancelled
  notes NVARCHAR(MAX),
  created_at DATETIME2 DEFAULT GETUTCDATE(),
  updated_at DATETIME2 DEFAULT GETUTCDATE(),
  FOREIGN KEY (lessor_id) REFERENCES fri_lessors(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS fri_invoices (
  ...
  paid_date DATE,
  payment_method NVARCHAR(50), -- card, bank_transfer, cash, mobilepay
  notes NVARCHAR(MAX),
...
CREATE TABLE IF NOT EXISTS fri_pages (
  ...
  layout_json NVARCHAR(MAX), -- JSON serialized layout data
  layout_data NVARCHAR(MAX), -- Normalized layout data
  created_at DATETIME2 DEFAULT GETUTCDATE(),
  updated_at DATETIME2 DEFAULT GETUTCDATE(),
...
CREATE TABLE IF NOT EXISTS fri_page_blocks (
  ...
  lessor_id UNIQUEIDENTIFIER NOT NULL,
  block_type NVARCHAR(100) NOT NULL, -- hero, text, pricing, vehicles, booking, contact, image, cta, testimonial, footer
  position INT NOT NULL,
  config_json NVARCHAR(MAX), -- JSON with block configuration
  block_data NVARCHAR(MAX), -- Structured block data
  created_at DATETIME2 DEFAULT GETUTCDATE(),
  updated_at DATETIME2 DEFAULT GETUTCDATE(),
...
-- Custom domains
CREATE TABLE IF NOT EXISTS fri_custom_domains (
  id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
...
CREATE TABLE IF NOT EXISTS fri_api_keys (
  ...
  service_type NVARCHAR(100), -- stripe, twilio, sendgrid, mapbox
  is_active BIT DEFAULT 1,
...
-- ============================================
-- 9. ROW LEVEL SECURITY (RLS) - Azure SQL specific implementation
-- ============================================
-- NOTE: Azure SQL uses security policies - implement in application layer or stored procedures
-- Example: Each Azure Function should validate lessor_id from JWT token and filter accordingly
