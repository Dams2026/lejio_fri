# AUTOFIQ → AUTOFIQ: Genbrugskort (funktioner/hooks)

**Mål:** Genbrug logik/UI fra AUTOFIQ, men **erstat altid data‑layer med Azure API** for Fri. **Ingen Supabase i Fri.**
...
- `src/hooks/useAuth.tsx` (AUTOFIQ/Supabase auth) → **Fri har `useFriAuth`**
- `src/hooks/useAdminAuth.tsx` (AUTOFIQ admin, Supabase) → **Fri admin auth separat**
- AUTOFIQ‑specifikke dashboards/routes

## 4) Anbefalet rækkefølge (genbrug fra AUTOFIQ → Fri)
...
**Næste skridt:** Vælg hvilken AUTOFIQ‑funktion vi vil genbruge først, så laver jeg den konkrete Fri‑version med Azure API.
