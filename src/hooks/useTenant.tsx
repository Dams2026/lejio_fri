 ...
 * Examples:
 * - martinbiludlejning.autofiq.dk → martinbiludlejning
 * - autofiq.dk → null (root domain)
 * - localhost:5173 → null (local dev)
 * - gray-hill-06c5ae603.6.azurestaticapps.net → null (Azure SWA root)
 * - autofiq.onrender.com → null (root domain, not a tenant)
 * - martinbiludlejning.autofiq.onrender.com → martinbiludlejning
 */
function extractSubdomain(): string | null {
  ...
  if (hostname.endsWith('.onrender.com')) {
    // Root domain like autofiq.onrender.com should not be treated as a tenant
    const subdomain = parts[0];
    if (subdomain === 'autofiq') {
      return null; // This is the root domain, not a tenant subdomain
    }
    // But martinbiludlejning.autofiq.onrender.com should return the full first part
    // Only if it's not the main platform domain
    return subdomain;
  }

  // Production: *.autofiq.dk
  if (hostname.endsWith('autofiq.dk')) {
    const subdomain = parts[0];
    return subdomain !== 'www' && subdomain !== 'autofiq' ? subdomain : null;
  }
